...
def calcular_gs_minima_para_materia(nota_1s: float, nota_cp_2s: float, meta_anual: float) -> float:
...
def get_calculation(calculation_id: int, db: Session = Depends(get_db)):
    calc = db.query(GradeCalculation).filter(GradeCalculation.id == calculation_id).first()
